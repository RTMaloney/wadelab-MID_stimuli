function MID_Dots_CD_1 (PresentControl)
...
%R Maloney July 2015
% Inputs:
%   PresentControl: enter true to display the CD control, false to display actual CD (this is the default).
%                   the CD control has the same disparities but they are shuffled over time, so there is no smooth motion-in-depth;
%                   it instead resembles a noisy, flickering 3D 'cloud'.
%
% Parameters modified to suit amblyopic patients, Jan 2017
...
%specify dot size:
dot_sigma_in_degrees = 0.075; %0.075 for amblyopes %size of SD of the dot profile in degs/vis angle
...
try %Start a try/catch statement, in case something goes awry with the PTB functions
    ...
    frequency = 1.4; %1.4 Hz based on thresholds of RM and JA %temp frequency of motion, in Hz. We want 1/2 cycle in 500 ms, so 1 Hz
    ...
    CD_disparity = 11/60 * PPD; %11 arcmin, based on RM and JA thresholds; disparity in pixels, defined in arcmin (akin to the amplitude of the sine wave)
    ...
    SineWave =  CD_disparity * sin(angFreq * t); %CD_disparity * sin(angFreq * t)
    ...
    % Now, if displaying the CONTROL stimulus, we simply suffle the frames temporally.
    % This gives us a stimulus with the same distribution of disparities but no consistent/smooth
    % change in disparity over time
    % This should be all we need to change for the CD stimulus control
    if PresentControl
        ShuffledFrames = randperm(FrmsFullCyc);
