function MID_Dots_IOVD_3 (PresentControl, ControlType)
...
%
% version _3: originally written for testing the updated version of the new IOVD control,
% as developed following the Jan 2016 MID meeting.
% In the new new IOVD control, the 2 eyes' dots are independent (in fact the same as in real IOVD), but dots move in opposite
% monocular directions in the 2 eyes (previously, a single set of dots was displayed to both eyes)
% Version_3 includes a switch that allows the user to display IOVD or the IOVD control.
% Viewed without goggles and binocularly IOVD & the IOVD control should be indistinguishable.
% However, viewed monocularly & with the goggles on will allow the viewer to tell whether it's the control or not
% (ie they'd see 2 directions for the control, 1 single direction for IOVD).
% This version also includes variables to define the L & R eye dot contrast separately
% (in case the user only wants to see what is happening in one eye; set contrast =0).
% R Maloney, 22 January 2016.
%
% *** Modified 12 March 2016 *** : there was a bug in the assignment of colour to the right eye dots.
% The right eye dots were sorted according to their nearest distances to any Left eye dot.
% The dots were sorted in the dot position matrix so that the closest L & R eye dots corresponded by rows in the dot position matrix.
% Then dot colour was assigned in an alternating manner (ie BWBWBW...; WBWBW) in the L & R eye matrices that defined colour.
% However, this sorting of the Right eye dots occured on every frame. Because 1/3 of dots change position on each frame
% (to give the max dot lifetime of 3 frames = 50 ms), sometimes the sorting changed within a dot's lifetime (as you would expect),
% which meant that a proportion of the dots changed colour during their lifetime, causing a visible flicker.
% Because the Left eye dots were not sorted, Left eye dots were not affected by this bug (ie no flicker in the Left eye).
% So now in this modified & updated version of version _3, closest dot distances are measured but dot lifetime is taken into account, such that
% once a dot has been assigned a colour (on its first frame), this colour is retained across its lifetime.
% A matrix of indices that provide the sorting of dots is now computed for every frame in the cycle: 'MinDist'.
% Indices in MinDist account for dot lifetime. MinDist is then used to sort dot colour assignment for each frame in the cycle.
% Dot colour must first be determined in a regular, alternating order for L/R eyes: ie BWBWBW...; WBWBW for each frame 
% in the matrices DotsIdxL and DotsIdxR.
% Previously we did this once and used the same matrix for every frame. Now we have to do it for each frame in the cycle
% and make sure it is indexed appropriately as the dots are drawn.
% MinDist simply sorts the dot colour matrices DotsIdxL and DotsIdxR. Because they are originally arranged in a regular,
% alternating & opposite order, sorting both matrices using the same set of indices in MinDist means that we end up with 
% two opposite matrices that define dot colour for each eye while keeping it constant across dot lifetime.
%
% Inputs:
%   PresentControl: enter true to display the IOVD control, false to display actual IOVD (this is the default)
%   ControlType: enter 1 for the 'new' IOVD control, where dots in BOTH eyes move in BOTH directions (the default),
%               or enter 2 for the 'original' IOVD control, which is identical to IOVD except the dots in both
%               eyes move coherently in the SAME direction

% Indicate the type of IOVD control if not parsed.
...
% Set the flag for the control type (if used)
if nargin < 2 || isempty (ControlType)
    ControlType = 1; % the 'new IOVD control is default
end

% Set the flag for presenting the control stimulus
if nargin < 1 || isempty (PresentControl)
...
try %Start a try/catch statement, in case something goes awry with the PTB functions
    ...
    frequency = 1.1; %1.1 Hz based on RM & JA thresholds; temp frequency of motion, in Hz. We want 1/2 cycle in 500 ms, so 1 Hz
    ...
    IOVD_disparity = 102/60 * PPD; %102 arcmin based on RM & JA thresholds; disparity in pixels, defined in arcmin (akin to the amplitude of the sine wave)
    ...
    if PresentControl  % to present the IOVD control stimulus
        
        if ControlType == 1 % this is for the 'new' control, with dots moving in both directions (single eye).
        ...
        elseif ControlType == 2 % For the *original* IOVD control; which is identical to IOVD except dots in both eyes move in SAME direction simultaneously
    ...
    else % Don't present any control; present actual IOVD stimulus.
